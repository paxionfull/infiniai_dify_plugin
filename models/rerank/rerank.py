...
class InfiniaiRerankModel(RerankModel):
    def _invoke(
        self,
        model: str,
        credentials: dict,
        ...
        credentials["api_key"] = credentials["infiniai_api_key"].strip()
        if len(docs) == 0:
            return RerankResult(model=model, docs=[])
        base_url = credentials.get("base_url", "https://cloud.infini-ai.com/maas/v1")
